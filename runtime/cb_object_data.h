...
//
// Ported from cbEnchanted's CBObject (src/cbobject.cpp) + ObjectInterface
// (src/objectinterface.cpp). The Rust port may pick its own in-memory layout;
// only the observable behaviour must match. Angles are degrees, 0° = right,
// growing clockwise in screen terms (Y is flipped at draw time, as in cb_map).
//
...
// GetAngle2(a, b) and PointObject(a→b) share one formula (objectinterface.cpp:
// 138, 620): the angle from `a` to `b` in degrees. Note the mixed signs and the
// /π·180 (degrees) — reproduced verbatim so headings match cbEnchanted exactly.
...
// Distance2(a, b): plain Euclidean distance between the two centres
// (objectinterface.cpp:631). Symmetric.
...
// ─── MoveObject heading (cbobject.cpp:308) ──────────────────────────────
...
// ─── TurnObject 0..360 wrap (cbobject.cpp:328) ──────────────────────────
// Relative one-shot rotate; keeps the angle in [0, 360]. Faithful to the
// cbEnchanted `>360` / `<0` while-loops (note: not `>=360`).
...
// ─── Animated-frame slice (cbobject.cpp:404-413) ────────────────────────
// Source cell for `frame` of an animated object sheet. `framesX = textureW /
// frameW`; `col = frame % framesX`; `row = frame / framesX` (this object path
// is already correct in cbEnchanted, unlike the cbimage.cpp Phase-1 slice — we
// keep the corrected /framesX, *frameHeight math). `frame` is 0-based and taken
...
// ─── ObjectSizeX / ObjectSizeY (objectinterface.cpp:652-688) ────────────
...
// to the nearest int (cbEnchanted's `int32_t(size + 0.5f)`).
...
// ─── Animation advance + life (cbobject.cpp:246-279) ────────────────────
...
// object should be auto-deleted). Faithful to cbEnchanted's `--life; if (life <=
// 0)` on an unsigned counter — a life that starts at 0 underflows and does NOT
// delete (a cbEnchanted quirk preserved deliberately).

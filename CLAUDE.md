...
### AOT codegen & linking (FD-048, FD-049)

With `--features llvm`, `cb --backend llvm <file>.cb [-o <out>]` compiles to a **native executable** and behaves like `--backend interp`: the full `cb_ir::Program` is lowered to native code (FD-049 — every `InstKind` and terminator: scalars, strings, arrays, user Types, value structs, function pointers) and linked against the full runtime closure. (FD-048 built the toolchain/link path first against a fixed empty `main`; FD-049 replaced that stub with real IR→LLVM lowering.) The artifact defaults to the source stem + the platform exe suffix, next to the source; `-o`/`--output` overrides it.

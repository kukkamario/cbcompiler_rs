...
- **Registry & ids**: lookup is `std::map<int32, CBObject*> objectMap`; draw order is **not** a list but four head/tail pointers (`firstObject/lastObject`, `firstFloorObject/lastFloorObject`) threaded through each object's intrusive `afterObj/beforeObj`. Ids come from `nextObjectId()` — a `static` counter, **monotonic from 1, never reused, shared across Objects + Maps + particles** (maps go in the same `objectMap` via `addMap`). **Port recommendation:** under `unsafe_code="deny"`, replace the intrusive list with a `slotmap`/`IndexMap` keyed by id + an explicit `Vec<ObjectId>` per draw chain (floor, regular) — behavior-equivalent, idiomatic.
- **Enumeration** (`InitObjectList`/`NextObject`): a **single shared, stateful iterator** (`iter = objectMap.begin()`; `NextObject` returns `iter->first` then `++iter`, 0 at end). Walks **id order** and **also surfaces map ids**. Non-reentrant.
...
- `DeleteObject` removes from pickables + collision checks + draw order, frees, erases from map. `ClearObjects` deletes all non-map objects, frees the tilemap, nulls all four draw heads.
...
- **Runtime threading model — RESOLVED: single-threaded** (verified 2026-06-19). The interpreter's `Value` model is built on `Rc`/`RefCell` (`crates/cb-backend-interp/src/{value,heap,string_handle}.rs`) with no `Arc`/`Mutex`/`thread::spawn`/`rayon` anywhere, so the VM is `!Send` and executes on one thread; runtime catalog/FFI calls happen on that same thread. Porting cbEnchanted's function-local `static` id counter, the single shared `InitObjectList` iterator, and the singleton tilemap as plain process-global state is therefore safe. (Revisit only if a future backend parallelizes VM execution.)

...
- **Tile animation = wall-clock time-step** (faithful to cbEnchanted: `currentFrame += elapsedSeconds / (slowness · animSpeed)`, reset to 0 once `(int)currentFrame > animLength` → a tile cycles `tile..tile+animLength` = `animLength+1` frames). Advances only while playing (`PlayObject(Map)` sets `animSpeed > 0`). *Corrected post-verification (2026-06-20):* the initial Phase-5c "deterministic fixed per-tick step" was frame-rate-dependent (far too fast with no `FrameLimit`) **and** its `[0,animLength)` wrap never advanced an `animLength==1` tile — so the real `cdm2.til` (7 `animLength==1` tiles) showed no animation. The pure advance/wrap math (`cb_map_advance_frame`) is now headless-unit-tested in `test_map.cpp`; only the wall-clock source lives in `cb_map.cpp`. Still moot for the in-tree fixtures (`testmap.til` has `animLength = 0`).

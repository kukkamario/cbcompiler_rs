...
- Float → Integer: implicit, **rounds half-up** — adds `0.5` then truncates (`(int)(x + 0.5)`), matching the `Int()` runtime function, **not** a straight truncation toward zero (so `10.5 → 11`, `-2.5 → -2`; see `cb_runtime.md` §Math). Both implicit conversions and explicit `Int(x)` use this same rule. The compiler emits a **narrowing-conversion warning** at the implicit conversion site; suppress by writing `Int(x)` explicitly.

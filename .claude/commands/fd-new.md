...
Always open the new FD file in Notepad++ at the end. Run:

```powershell
Start-Process notepad++ -ArgumentList 'docs/features/FD-{number}_{SLUG}.md'
```

If `notepad++` is not on PATH, fall back to the standard install location:

```powershell
Start-Process 'C:\Program Files\Notepad++\notepad++.exe' -ArgumentList 'docs/features/FD-{number}_{SLUG}.md'
```

Do this even when the user hasn't asked — it's the default for this command.
